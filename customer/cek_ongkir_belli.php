...
<select name="ongkir" form="formku"">
<?php
// print_r($data);
for ($k=0; $k < count($data['rajaongkir']['results']); $k++) {
	for ($l=0; $l < count($data['rajaongkir']['results'][$k]['costs']); $l++) {
 echo "<option value='".$data['rajaongkir']['results'][$k]['costs'][$l]['cost'][0]['value']."'>".$data['rajaongkir']['results'][$k]['costs'][$l]['service']." (".$data['rajaongkir']['results'][$k]['costs'][$l]['cost'][0]['etd']." Hari) --".$data['rajaongkir']['results'][$k]['costs'][$l]['cost'][0]['value']."</option>";
	}
}
?>
 </select>
